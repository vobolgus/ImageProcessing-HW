...
def create_vit_model(device, num_classes=2):
...
def run_vit_experiment():
    ...
    print("\n--- Step 2: Creating ViT-B/16 model ---")
    model_vit = create_vit_model(device, len(class_names))
    ...
    optimizer_vit, exp_lr_scheduler, criterion = setup_training(
        model_vit, LEARNING_RATE, NUM_EPOCHS, class_weights=class_weights
    )
